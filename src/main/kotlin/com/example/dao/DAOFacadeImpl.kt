...
class DAOFacadeImpl : DAOFacade {
    private fun resultRowArticle(row: ResultRow) = Article(
        id = row[Articles.id], title = row[Articles.title], body = row[Articles.body]
    )

    override suspend fun allArticles(): List<Article> = dbQuery {
        Articles.selectAll().map(::resultRowArticle)
    }

    override suspend fun article(id: Int): Article? = dbQuery {
        Articles.select { Articles.id eq id }.map(::resultRowArticle).singleOrNull()
    }
    ...
    override suspend fun addNewArticle(title: String, body: String): Article? = dbQuery {
        ...
        insertStatement.resultedValues?.singleOrNull()?.let(::resultRowArticle)
    }
